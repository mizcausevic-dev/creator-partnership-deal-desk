...
function renderHero(active: string) {
  ...
  <div class="herorow">
    <section class="hero">
      <div class="chiprow">
        <span class="meta-chip">Creator operations</span>
      ...
      <h1>Creator deal and launch control plane.</h1>
      <p>Deal changes, obligation blockers, and launch posture in one operator surface for creator and partnership teams that need faster launches without rights drift, approval chaos, or broken measurement.</p>
      ${renderTabs(active, links)}
    </section>
    <aside class="side">
      <div class="bluf">
        <div class="lbl">Current watch item</div>
        <p><strong>Paid scope drift</strong> and rights mismatches are the fastest way to turn a high-intent creator launch into a broken buyer-facing promise.</p>
      </div>
      <div class="corr">
        <div class="lbl">Operator correction</div>
        <p>Map every creator change to downstream approvals, usage rights, UTMs, and launch timing before the next packet becomes buyer-visible.</p>
      </div>
      <div class="toolchips">
        <span class="toolchip tc-codex">codex</span>
        <span class="toolchip tc-claude">claude</span>
        <span class="toolchip tc-gpt">revops</span>
...
function heroShell(active: string, content: string) {
  return `${renderHero(active)}${content}
    <footer>
      <div>creator-partnership-deal-desk · synthetic demonstration data only</div>
  ...
  return layout(
    ...
    heroShell(
      ...
      `<section class="section">
        <div class="sh"><h2>Control snapshot</h2><div class="note">Launch-safe creator operations</div></div>
        <div class="kpis">
          <div class="kpi cyan"><div class="v">${metrics.dealChanges}</div><div class="lbl">Deal changes</div><div class="h">Active partnership changes tied to owner, platform, and next action.</div></div>
          <div class="kpi red"><div class="v">${metrics.urgentChanges}</div><div class="lbl">Urgent changes</div><div class="h">Red changes where launch or rights risk is already building.</div></div>
          <div class="kpi amber"><div class="v">${metrics.blockedObligations}</div><div class="lbl">Blocked obligations</div><div class="h">Brief, legal, and measurement blockers still waiting on proof.</div></div>
          <div class="kpi plum"><div class="v">${metrics.riskyPackets}</div><div class="lbl">Risky packets</div><div class="h">Launch packets with visible go/no-go pressure.</div></div>
        </div>
        <div class="quote">
          <div class="lbl">Primary recommendation</div>
          <div class="q">${metrics.recommendation}</div>
          <div class="a">Buyer-safe creator launches start with aligned rights, approvals, and reporting proof.</div>
          <div class="qm">”</div>
        </div>
      </section>
      <section class="section">
        <div class="sh"><h2>Launch packets</h2><div class="note">What can ship next</div></div>
        <div class="inv">
          ${packets
            .map(
              (packet) => `<article class="ivc">
                ...
                <p>${packet.completenessScore}% complete · ${packet.launchWindowHours} hours to launch window.</p>
