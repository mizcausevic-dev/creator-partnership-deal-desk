...
<head>
  ...
  <title>${title}</title>
  <meta name="description" content="TypeScript deal desk for creator partnerships, deliverable governance, offer review, launch-safe approvals, and buyer-facing collaboration operations. Operator surface in the Kinetic Gain Suite." />
  ...
  <link rel="canonical" href="https://creators.kineticgain.com/" />
  <link rel="icon" type="image/svg+xml" href="${KG_FAVICON_DATA_URI}" />
  <meta property="og:type" content="website" />
  <meta property="og:site_name" content="Kinetic Gain" />
  <meta property="og:title" content="${title}" />
  <meta property="og:url" content="https://creators.kineticgain.com/" />
  <meta property="og:description" content="TypeScript deal desk for creator partnerships, deliverable governance, offer review, launch-safe approvals, and buyer-facing collaboration operations. Operator surface in the Kinetic Gain Suite." />
  <meta name="twitter:card" content="summary" />
  <meta name="twitter:title" content="${title}" />
  <meta name="twitter:description" content="TypeScript deal desk for creator partnerships, deliverable governance, offer review, launch-safe approvals, and buyer-facing collaboration operations. Operator surface in the Kinetic Gain Suite." />
  <script type="application/ld+json">{"@context":"https://schema.org","@graph":[{"@type":"ProfessionalService","@id":"https://creators.kineticgain.com/#service","name":"Kinetic Gain — TypeScript deal desk for creator partnerships, deliverable governance, offer review, launch-safe approvals, and buyer-facing collaboration operations","url":"https://creators.kineticgain.com/","areaServed":"United States","description":"TypeScript deal desk for creator partnerships, deliverable governance, offer review, launch-safe approvals, and buyer-facing collaboration operations. Operator surface in the Kinetic Gain Suite."},{"@type":"WebSite","@id":"https://creators.kineticgain.com/#website","url":"https://creators.kineticgain.com/","name":"TypeScript deal desk for creator partnerships, deliverable governance, offer review, launch-safe approvals, and buyer-facing collaboration operations","publisher":{"@type":"Organization","name":"Kinetic Gain LLC"}}]}</script>
  <style>${KG_STYLE01_CSS}${KG_COMPAT_CSS}${CREATOR_SURFACE_CSS}</style>
</head>
<body>
  <header>
    <div class="wrap nav">
      <a class="kg-logo" href="/" aria-label="Kinetic Gain — TypeScript deal desk for creator partnerships, deliverable governance, offer review, launch-safe approvals, and buyer-facing collaboration operations">
        ${KG_MARK_SVG}
        <span class="kg-word">Kinetic Gain</span>
      </a>
      <nav class="nav-links" id="primaryNav">
        <a href="/deal-lane">Deal Lane</a>
    ...
    <div class="wrap">
      <div class="foot-top">
        <div>
          ...
          <p class="foot-tag">TypeScript deal desk for creator partnerships, deliverable governance, offer review, launch-safe approvals, and buyer-facing collaboration operations — operator surface in the Kinetic Gain Suite. Static demo data; no live calls.</p>
        </div>
        <div class="foot-cols">
          <div class="foot-col">
            <h4>Estate</h4>
          ...
          <div class="foot-col">
            <h4>This surface</h4>
            <a href="https://github.com/mizcausevic-dev/creator-partnership-deal-desk">GitHub repo</a>
          </div>
        </div>
      </div>
      <div class="foot-bot">
...
function routePills(active: string, links: NavLink[]) {
  return links
    .map(
      (link) =>
        `<a class="${link.href === active ? "active" : ""}" href="${link.href}">${link.label}</a>`
    )
    .join("");
}
...
function renderHero(active: string) {
  ...
  const urgentChange = dealLane().find((item) => item.risk === "red") ?? dealLane()[0];
  const launchRisk = launchReadiness().find((item) => item.status !== "green") ?? launchReadiness()[0];
  const rightsBlock = obligationRisks().find((item) => item.readiness === "red") ?? obligationRisks()[0];
  ...
  <section class="hero reveal">
    <div class="acard hero-panel">
      <div class="hero-grid">
        <div class="hero-copy">
          <span class="eyebrow">Creator operations</span>
          <div class="hero-meta">
            <span class="meta-chip">Rights + approvals</span>
            <span class="meta-chip">Launch trust</span>
            <span class="meta-chip">Measurement continuity</span>
          </div>
          <h1>Where do creator deal changes quietly become launch and reporting risk?</h1>
          <p class="hero-subtle">Creator Partnership Deal Desk keeps creator-scope drift, approval blockers, usage rights, and measurement dependencies visible in one board-readable operator surface before the next packet becomes buyer-facing.</p>
          <div class="hero-nav">${routePills(active, links)}</div>
          <div class="hero-kpis">
            <div class="stat"><label>Deal changes</label><strong>${metrics.dealChanges}</strong><span>Active creator or partnership changes tied to owner, platform, and next move.</span></div>
            <div class="stat"><label>Urgent changes</label><strong>${metrics.urgentChanges}</strong><span>Red changes where rights, approvals, or launch timing are already at risk.</span></div>
            <div class="stat"><label>Blocked obligations</label><strong>${metrics.blockedObligations}</strong><span>Proof gaps still waiting on legal, analytics, or partnership confirmation.</span></div>
            <div class="stat"><label>Risky packets</label><strong>${metrics.riskyPackets}</strong><span>Launch packets with visible go / no-go pressure before the next public handoff.</span></div>
          </div>
        </div>
        <aside class="hero-aside">
          <div class="acard">
            <span class="metric-chip">Current watch item</span>
            <h3>${urgentChange.platform} · ${urgentChange.creator}</h3>
            <p><strong>${urgentChange.issueType}</strong> is the fastest path to a broken buyer-facing promise because ${urgentChange.excerpt.toLowerCase()}</p>
          </div>
          <div class="acard">
            <span class="metric-chip">Operator correction</span>
            <h3>What the team should do next</h3>
            <ul class="hero-mini-list">
              <li><strong>${urgentChange.owner}</strong><span>${urgentChange.nextAction}</span></li>
              <li><strong>${rightsBlock.owner}</strong><span>${rightsBlock.requiredEvidence}</span></li>
              <li><strong>${launchRisk.audience}</strong><span>${launchRisk.decisionNote}</span></li>
            </ul>
          </div>
          <div class="acard">
            <span class="metric-chip">Control surfaces</span>
            <p>Creator deals, approvals, usage rights, UTM maps, launch timing, and reporting continuity stay in one controlled view instead of drifting across briefs, comments, and last-minute packet fixes.</p>
          </div>
        </aside>
      </div>
    </div>
...
export function renderOverview() {
  const metrics = summary();
  const changes = dealLane();
  const risks = obligationRisks();
  const packets = launchReadiness();
  return layout(
    "Creator Partnership Deal Desk",
    heroShell(
      "/",
      `<section class="section">
        <div class="sh"><h2>Deal lane</h2><div class="note">Partnership pressure that leadership can actually act on</div></div>
        <div class="section-cards">
          ${changes
            .map(
              (item) => `<article class="acard surface-card">
                <span class="tag ${item.risk}">${item.risk}</span>
                <h3>${item.platform} · ${item.creator}</h3>
                <div class="surface-meta">
                  <p><strong>${item.changeId}</strong> · ${item.issueType}</p>
      ...
      <section class="section">
        <div class="sh"><h2>Launch pressure</h2><div class="note">Where proof, rights, and reporting can still break launch trust</div></div>
        <div class="signal-grid">
          <div class="acard">
            <h3>Obligation risks</h3>
            <ul class="signal-list">
              ${risks
                .map(
                  (item) =>
                    `<li><strong>${item.riskId} · ${item.blocker}</strong><br />${item.requiredEvidence}</li>`
                )
                .join("")}
            </ul>
          </div>
          <div class="acard">
            ...
            <ul class="signal-list">
              ${risks
                .map(
                  (item) =>
                    `<li><strong>${item.impactArea}</strong><br />${item.note}</li>`
                )
                .join("")}
            </ul>
          </div>
      ...
      <section class="section">
        <div class="sh"><h2>Launch packets</h2><div class="note">Go / hold / repair decisions before the next public handoff</div></div>
        <div class="packet-grid">
          ${packets
            .map(
              (packet) => `<article class="acard packet-card">
                <div class="packet-top">
                  <div class="packet-score">${packet.completenessScore}%</div>
                ...
                <ul class="packet-list">
                  ...
                  <li><strong>Window:</strong> ${packet.launchWindowHours} hours</li>
                </ul>
...
export function renderDealLane() {
  const rows = dealLane();
  return layout(
    "Creator Partnership Deal Desk — Deal Lane",
    heroShell(
      "/deal-lane",
      `<section class="section">
        <div class="sh"><h2>Deal lane</h2><div class="note">Partnership changes under pressure</div></div>
        <table class="ttbl">
          <thead><tr><th>Deal</th><th>Owner</th><th>Risk</th><th>Next move</th></tr></thead>
          <tbody>
            ${rows
              .map(
                (item) => `<tr>
                  <td><b>${item.platform} · ${item.creator}</b><br />${item.changeId} · ${item.issueType}<br />${item.excerpt}</td>
                  <td>${item.owner}</td>
...
export function renderObligationRisks() {
  const risks = obligationRisks();
  return layout(
    "Creator Partnership Deal Desk — Obligation Risks",
    heroShell(
      "/obligation-risks",
      `<section class="section">
        <div class="sh"><h2>Obligation risks</h2><div class="note">Evidence gaps before launch</div></div>
        <div class="ledger">
          <div class="led">
...
export function renderLaunchReadiness() {
  const packets = launchReadiness();
  return layout(
    "Creator Partnership Deal Desk — Launch Readiness",
    heroShell(
      "/launch-readiness",
      `<section class="section">
        <div class="sh"><h2>Launch readiness</h2><div class="note">Go / hold / repair decisions</div></div>
        <div class="board">
          ${packets
            .map(
              (packet) => `<article class="pcard">
                ...
                <ul class="check">
                  ...
                  <li>Window: ${packet.launchWindowHours} hours</li>
                </ul>
...
export function renderVerification() {
  ...
  return layout(
    "Creator Partnership Deal Desk — Verification",
    heroShell(
      "/verification",
      `<section class="section">
        ...
        <div class="ledger">
          ...
          <div class="led">
            ...
            <ul>
              <li><span class="pip">→</span><div>Creator, sponsorship, and campaign data can surface in-product without opening unsafe write paths.</div></li>
              <li><span class="pip">→</span><div>Approval posture and rights proof stay connected to buyer-facing dashboards.</div></li>
              <li><span class="pip">→</span><div>Synthetic data only; this repo demonstrates system shape, not production customer data.</div></li>
            </ul>
          </div>
        </div>
      </section>`
...
export function renderDocs() {
  return layout(
    "Creator Partnership Deal Desk — Docs",
    heroShell(
      "/docs",
      `<section class="section">
        <div class="sh"><h2>System docs</h2><div class="note">Routes and control surfaces</div></div>
        <div class="stack">
          <article class="src"><div class="src-name">Control plane</div><div class="src-tit">Deal lane</div><p>Partnership-by-partnership queue with owner routing, issue type, and next action.</p></article>
          <article class="src"><div class="src-name">Risk map</div><div class="src-tit">Obligation risks</div><p>Required evidence, owner lanes, and impact areas for blocked approvals and rights drift.</p></article>
          <article class="src"><div class="src-name">Launch view</div><div class="src-tit">Launch readiness</div><p>Packet-level completeness, blocker state, and launch-window timing.</p></article>
          <article class="src"><div class="src-name">Embedded tie-back</div><div class="src-tit">KG Embedded</div><p>Security-first in-product analytics extension for partnership, sponsorship, and growth workflows.</p></article>
        </div>
        <div class="quote">
          <div class="lbl">Route surface</div>
          <div class="q"><code>/</code> · <code>/deal-lane</code> · <code>/obligation-risks</code> · <code>/launch-readiness</code> · <code>/verification</code> · <code>/docs</code></div>
          <div class="a">The shell is now standardized for future operator surfaces.</div>
          <div class="qm">”</div>
        </div>
      </section>`
    )
